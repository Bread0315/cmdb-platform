...
def user_list():
    db = get_db()
    users = db.execute("SELECT id, username, real_name, email, role, is_active, must_change_password, created_at FROM users ORDER BY created_at DESC").fetchall()
...
def user_delete(uid):
    ...
    # 清理关联数据
    db.execute("UPDATE device_logs SET user_id=NULL WHERE user_id=?", (uid,))
    db.execute("UPDATE ip_pools SET created_by=NULL WHERE created_by=?", (uid,))
    db.execute("UPDATE business_systems SET created_by=NULL WHERE created_by=?", (uid,))
