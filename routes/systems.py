    ...
    if request.method == "POST":
        ...
        if not data["name"]:
            ...
            """, (data["name"], data["sys_type"], data["status"], data["department"], data["owner"],
                  ...
                  data["remark"], sid))
            # 记录旧的设备关联
            old_rels = db.execute("SELECT device_id FROM system_device_rel WHERE system_id=?", (sid,)).fetchall()
            old_device_ids = {r['device_id'] for r in old_rels}
            ...
            # 记录关联变更
            new_device_ids = {int(d) for d in device_ids}
            added = new_device_ids - old_device_ids
            removed = old_device_ids - new_device_ids
            if added:
                log_to_db(db, 'INFO', '业务系统', '新增设备关联', f"系统 {data['name']} 新增关联 {len(added)} 台设备")
            if removed:
                log_to_db(db, 'INFO', '业务系统', '移除设备关联', f"系统 {data['name']} 移除关联 {len(removed)} 台设备")
