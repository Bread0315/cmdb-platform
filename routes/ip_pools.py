...
def ip_reserve(pid):
    ...
    if not ip:
        ...
        if not row:
            ...
            log_to_db(db, 'INFO', 'IP管理', '分配IP', f"分配IP {ip} (类型:{ip_type or '未指定'})")
            db.commit()
...
def ip_batch_reserve(pid):
    ...
    db.commit()
    if success_count > 0:
        log_to_db(db, 'INFO', 'IP管理', '批量分配IP', f"批量分配 {success_count} 个IP")
...
def ip_batch_release(pid):
    ...
    db.commit()
    if success_count > 0:
        log_to_db(db, 'INFO', 'IP管理', '批量释放IP', f"批量释放 {success_count} 个IP")
...
def ip_release(pid):
    ...
    if not ip:
        ...
        if not row:
            ...
            db.execute("UPDATE ip_addresses SET status='available', device_id=NULL, ip_type='', remark='' WHERE id=?",
                       (row['id'],))
            log_to_db(db, 'INFO', 'IP管理', '释放IP', f"释放IP {ip}")
            db.commit()
