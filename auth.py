...
def is_account_locked(username):
    ...
    row = db.execute(
        "SELECT COUNT(*) as cnt FROM login_logs WHERE username=? AND success=0 AND created_at>=?",
        (username, cutoff)
    ).fetchone()
...
def get_lockout_remaining(username):
    ...
    if row:
        ...
        last = datetime.strptime(created_at, '%Y-%m-%d %H:%M:%S')
