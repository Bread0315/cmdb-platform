...
# --------------- 创建应用 ---------------
app = Flask(__name__)
# Secret Key: 必须从环境变量读取，禁止使用默认值
_secret = os.environ.get("CMDB_SECRET")
if not _secret:
    # 首次运行时自动生成并保存
    import secrets as _secrets
    _secret = _secrets.token_hex(32)
    os.environ["CMDB_SECRET"] = _secret
    # 写入.env文件
...
app.config.update(
    ...
    SESSION_COOKIE_SECURE=os.environ.get("CMDB_HTTPS", "false").lower() == "true",
    PERMANENT_SESSION_LIFETIME=timedelta(hours=SESSION_TIMEOUT_HOURS),
    WTF_CSRF_ENABLED=True,
    WTF_CSRF_TIME_LIMIT=3600,
    MAX_CONTENT_LENGTH=10 * 1024 * 1024,  # 10MB上传限制
)
...
def init_setup():
    ...
    try:
        ...
        return jsonify({"success": False, "message": "初始化失败，请检查配置后重试"})
...
def inject_user():
    if 'user_id' in session:
        try:
            ...
            user_fields = "id, username, real_name, email, role, is_active, must_change_password, created_at"
            if config.get("db_type") == "mysql":
                cur = db.cursor()
                cur.execute(f"SELECT {user_fields} FROM users WHERE id=%s", (session['user_id'],))
                user = cur.fetchone()
            else:
                user = db.execute(f"SELECT {user_fields} FROM users WHERE id=?", (session['user_id'],)).fetchone()
...
if __name__ == "__main__":
    ...
    debug_mode = os.environ.get("CMDB_DEBUG", "false").lower() == "true"
    import werkzeug
